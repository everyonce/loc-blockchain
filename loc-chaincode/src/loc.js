...
let LetterStatus = class {
  TERMS_APPROVAL = 0;
  SELLER_ACCEPTANCE = 1;
  SELLER_BANK_APPROVAL = 2;
  SHIPPED = 3;
  BUYER_BANK_APPROVAL = 4;
  SHIPMENT_RECEIVED = 5;
  SELLER_ACCEPTANCE = 6;
  PAID = 7;
  CLOSED = 8;
...
let Chaincode = class {
   ...
   *   "letterStatus": "STATUS" - note this will always be started as TERMS_APPROVAL
   *   "buyerId": "fgf-dfgg-dgdg", -guid
   *   "buyerBankId": "wfw-234-sdfgs", - guid
   *   "sellerId": "345dfgfe34t", - guid
   *   "sellerBankId": "23423-2342-242", - guid
   *   "productDetails": [] - see product structure below
   *   "rules": [], - see rule structure below
   *   "approvals": [], - see approval structure below
   *   "letterDate": "2018-09-20T12:41:59.582Z",
  ...
  async createLetter(stub, args) {
    ...
    let key = 'letter.' + json['letterId'];

    json['docType'] = 'letter';
    json['letterStatus'] = LetterStatus.TERMS_APPROVAL;

    console.log('##### createLetter payload: ' + JSON.stringify(json));

    // Check if the letter already exists
    let letterQuery = await stub.getState(key);
    if (letterQuery.toString()) {
      throw new Error('##### createLetter - This letter already exists: ' + json['letterId'] + '/' + json['buyerId']);
    }
    await stub.putState(key, Buffer.from(JSON.stringify(json)));
    console.log('============= END : createLetter ===========');
  }
   ...
   * 
   * 
   * @param {*} aregs - the initial letter of credit. This will be JSON, as follows:
   * {
   *   "letterId": "1231-sdfsd-1231-dsfw",  -guid
   *   "letterStatus": "STATUS" - note this will always be started as TERMS_APPROVAL
   * }
   */
  async changeLetterStatus(stub, args) {
    console.log('============= START : changeLetterStatus ===========');
    let json = JSON.parse(args);
    //let json = JSON.parse(args);
    let key = 'letter.' + json['letterId'];

    console.log('##### changeLetterStatus payload: ' + JSON.stringify(json));

    // Check if the donor already exists
    let letterQuery = await stub.getState(key);
    if (!letterQuery.toString()) {
      throw new Error('##### createDonor - This letter does not exist: ' + json['letterId'] );
    }
    let letterJson = JSON.parse(letterQuery);
    letterJson['letterStatus'] = json['letterStatus'];

    await stub.putState(key, Buffer.from(JSON.stringify(letterJson)));
    console.log('============= END : changeLetterStatus ===========');
  }
